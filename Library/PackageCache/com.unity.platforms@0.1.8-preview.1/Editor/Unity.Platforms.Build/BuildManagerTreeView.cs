...
namespace Unity.Platforms.Build
{
    ...
    {
        BuildInstructions m_BuildInstructions;
        ...
        internal BuildTreeViewItem(int depth, BuildInstructions buildInstructions) : base(buildInstructions.BuildSettings.GetInstanceID(), depth)
        {
    ...
    internal class BuildManagerTreeView : TreeView
    {
        ...
        private Dictionary<int, BuildTreeViewItem> m_CachedRowMap = new Dictionary<int, BuildTreeViewItem>();
        ...
        private void DisplayItem(BuildTreeViewItem item, int c, Rect rc)
        {
            ...
            switch (column)
            {
                case BuildManagerTreeState.Columns.BuildSettings:
                    var pipeline = props.BuildSettings.GetBuildPipeline();
                    var pipelineName = pipeline == null ? "<No pipeline>" : pipeline.name;
                    if (GUI.Button(rc, props.BuildSettings.name + " with " + pipelineName, Styles.buildSettingsButton))
                    {
                        EditorGUIUtility.PingObject(props.BuildSettings);
                    }
            ...
            if (EditorGUI.EndChangeCheck())
                this.SetSelection(new List<int>(new[] { props.BuildSettings.GetInstanceID() }));
        }
