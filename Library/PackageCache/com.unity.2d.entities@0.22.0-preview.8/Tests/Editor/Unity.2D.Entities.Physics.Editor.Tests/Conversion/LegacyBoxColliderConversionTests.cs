...
[TestFixture]
class LegacyBoxColliderConversionTests : BaseLegacyColliderConversionFixture
{
    [Test]
    public void BoxCollider_BadTransformScale_ConversionTest()
    {
        // Set-up the hierarchy.
        {
            ...
            CreateLegacyComponent<BoxCollider2D>(Root);
        }
    ...
    [Test]
    public void BoxCollider_Root_ConversionTest()
    {
        var expectedOffset = new float2(3f, -4f);
        var expectedSize = new float2(5f, 6f);

        // Set-up the hierarchy.
        {
            Root = new GameObject();

            var collider = CreateLegacyComponent<BoxCollider2D>(Root);
        ...
        RunConversionTest(
            (BlobAssetReference<Unity.U2D.Entities.Physics.Collider> colliderBlob) =>
            {
                ...
                Assert.AreEqual(ColliderType.Box, colliderBlob.Value.ColliderType);
                ...
                PhysicsAssert.AreEqual(expectedSize, collider.Size, Epsilon);
            });
    }

    [Test]
    public void BoxCollider_Child_ConversionTest(
        ...
        [Values(0, 45f, -30f, 140f, -280f)] float rotateZ)
    {
        ...
        // Set-up the hierarchy.
        {
            CreateHierarchy<Rigidbody2D, BoxCollider2D>();
