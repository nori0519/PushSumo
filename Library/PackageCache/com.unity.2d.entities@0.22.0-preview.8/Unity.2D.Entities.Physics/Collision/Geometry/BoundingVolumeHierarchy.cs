...
namespace Unity.U2D.Entities.Physics
{
    ...
    public partial struct BoundingVolumeHierarchy
    {
        ...
        private static unsafe void ProcessAA<T>(ref Node node, int4* compressedData, ref int* stack, ref int* stackA, ref int* stackB, ref T pairWriter) where T : struct, ITreeOverlapCollector
        {
            ...
            FourAabb nodeBounds = node.Bounds;

            FourAabb* aabbT = stackalloc FourAabb[3];
            ...
            ref int* stackA, ref int* stackB, ref T pairWriter) where T : struct, ITreeOverlapCollector
        {
            ...
            FourAabb aabbTA = nodeA->Bounds.GetAabbT(0);
                ...
                for (int i = 0; i < 4; i++)
                {
                    if (compressedCount[i] > 0)
                    {
                        *((int4*)stackA) = compressedData[i];
                        ...
                        FourAabb aabbT = nodeA->Bounds.GetAabbT(i);
            ...
            where TCollector : struct, ICollector<OverlapPointHit>
        {
            ...
            *top++ = 1;
            do
            {
                ...
                bool4 hitMask = node->Bounds.OverlapPoint(input.Position);
                int4 hitData;
            ...
            where TCollector : struct, ICollector<OverlapColliderHit>
        {
            ...
            *top++ = 1;
            ...
            {
                Node* node = m_Nodes + *(--top);
                bool4 hitMask = node->Bounds.Overlap(ref colliderAabb);
                int4 hitData;
                int hitCount = math.compress((int*)(&hitData), 0, node->Data, hitMask);
            ...
            where TCollector : struct, ICollector<DistanceHit>
        {
            ...
            *stack++ = 1;
            ...
            do
            {
                ...
                bool4 overlap = (node->Bounds.MinX <= node->Bounds.MaxX) & (distanceToNodesSquared <= maxDistanceSquared);
                int4 hitData;
